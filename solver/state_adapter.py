...
class StateAdapter:
    ...
    def adapt(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        elif "WindSpeed" in data:
            out["env_wind_speed"] = float(data["WindSpeed"])

        if "env_WaveElev" in data:
            out["env_wave_elev"] = float(data["env_WaveElev"])
        elif "WaveElev" in data:
            out["env_wave_elev"] = float(data["WaveElev"])
        ...
        elif "AeroThrust" in data:
            out["thrust"] = float(data["AeroThrust"])

        if "env_GenTorque" in data:
            out["gen_torque"] = float(data["env_GenTorque"])
        elif "GenTq" in data:
            out["gen_torque"] = float(data["GenTq"])
